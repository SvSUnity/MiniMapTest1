...
public class MinimapIconManager : MonoBehaviour
{
    ...
    void CreateTeamIcon(int teamCnt)
    {
        ...
        {
            Image minimapIcon = Instantiate(teamIcon) as Image;
            minimapIcon.transform.SetParent(transform);
            minimapIcon.gameObject.SetActive(false);
    ...
    public Image GetMinimapIcon(string tag)
    {
        ...
        switch(tag)
        {
            ...
            case "TeamPlayer":
                ...
                {
                    if (teamIcons[i].gameObject.activeSelf == false)
                    {
                        reqObject = teamIcons[i];
                        break;
                    }
                }
                //초기설정 개수보다 부족할경우 추가생성
                if (reqObject == null)
                {
                    Image newObject = Instantiate(teamIcon) as Image;
                    newObject.transform.SetParent(transform);
                    teamIcons.Add(newObject);
                    reqObject = newObject;
                }
                break;
        }
